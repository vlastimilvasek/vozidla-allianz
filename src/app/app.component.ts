...
export class AppComponent implements OnInit, OnDestroy {
    ...
    submitOsobni(form: any): void {
        // console.log('valid udajeForm: ', this.udajeForm.valid);
        // console.log('valid objektForm: ', this.objektForm.valid);
        // console.log('vprodukt: ', this.vprodukt);
        if (form.valid && this.udajeForm.valid && this.objektForm.valid && this.vprodukt) {
            this.staticTabs.tabs[5].active = true;
        } else {
            if (!this.vprodukt) { this.staticTabs.tabs[1].active = true; }
            if (!this.udajeForm.valid) { this.staticTabs.tabs[2].active = true; }
            if (!this.objektForm.valid) { this.staticTabs.tabs[3].active = true; }
        }
    }

    sjednat(form: any): void {
        if (form.valid && this.udajeForm.valid && this.objektForm.valid && this.osobniForm.valid && this.vprodukt) {
            this.layout.form_r.loading = true;
            this.paramsService.ulozSjednani(this.data).subscribe({
                next: (sjednani) => {
                    console.log('sjednat - resp: ', sjednani);
                    if ( sjednani.status ) {
                        this.data.sid = sjednani.sid;
            ...
            if (!this.udajeForm.valid) { this.staticTabs.tabs[2].active = true; }
            if (!this.objektForm.valid) { this.staticTabs.tabs[3].active = true; }
            if (!this.osobniForm.valid) { this.staticTabs.tabs[4].active = true; }
        }
    }
    ...
    tabSrovnani(): void {
        // console.log('APP - tabSrovnani ', !this.offers.length  && !this.layout.kalkulaceAktivni  && this.zadaniForm.valid);
    ...
    kalkuluj(): void {
        ...
        this.paramsService.ulozKalkulaci(this.data).subscribe(
            resp => { 
                console.log('APP - kalkuluj - ulozeni kalkualce: ', resp);           
            });
